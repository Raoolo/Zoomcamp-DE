# previously called yellow_taxi.py 
import argparse
import os 
import pandas as pd
...
def main(params):
    ...
    lookup_table_name = params.lookup_table_name      # a lookup table is a table that contains columns that are referenced by other tables, so that in the original table you only store the id and not the name which is instead stored in the lookup table
    ...
    print("Successfully uploaded data to PostgreSQL DB")
